...
import { uploadToR2, deleteFromR2, urlToR2Key } from '@/lib/r2';
...
export async function deleteMediaAction(mediaId: string) {
  ...
  const item = await prisma.mediaItem.findFirst({ where: { id: mediaId, tenantId } });
  if (item) {
    const key = urlToR2Key(item.url);
    if (key) deleteFromR2(key).catch(console.error);
