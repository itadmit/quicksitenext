...
export function DataTable({ headers, children }: Props) {
  return (
    <div className="overflow-x-auto rounded-2xl bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
        ...
        <thead>
          <tr className="border-b border-slate-100 bg-slate-50/40">
            {headers.map((h, i) => {
              ...
              return (
                <th key={i} className={`px-5 py-3.5 text-[11px] font-semibold uppercase tracking-wider text-slate-400 ${cls}`}>
                  {label}
                </th>
              );
            })}
...
export function DataTableCell({ children, className = '', dir }: { children?: React.ReactNode; className?: string; dir?: 'ltr' | 'rtl' }) {
  return <td className={`px-5 py-3.5 text-[13px] ${className}`} dir={dir}>{children}</td>;
}
